 ...
 * First-Fit Scheduler
 * 
 * Bin packing algorithm that: 1. Iterates through nodes in a fixed order 2.
 * Assigns pod to the FIRST node that has sufficient resources 3. No
 * optimization for load balancing
 * 
 * Faster than Best-Fit but may lead to fragmentation. Used as baseline
 * comparison for enhanced multi-criteria scheduler.
 */
public class FirstFitScheduler {
	private static final Logger logger = Logger.getLogger(FirstFitScheduler.class.getName());
	...
	private static class SchedulingMetrics {
		...
		public SchedulingMetrics(String podName, Duration totalTime, Duration selectionTime, Duration bindingTime,
				String selectedNode, double nodeUtilizationCpu, double nodeUtilizationMemory) {
		...
		public void logMetrics() {
			...
			logger.info(String.format("- Algorithm: First-Fit"));
	...
	public static void main(String[] args) throws IOException {
		try {
			InputStream loggingConfigStream = FirstFitScheduler.class.getClassLoader()
			...
			logger.info("Initializing First-Fit Scheduler...");
			...
			while (true) {
				try {
					...
					if (!unscheduledPods.isEmpty()) {
						...
						nodes.sort(Comparator.comparing(node -> node.getMetadata().getName()));
						...
						for (V1Pod pod : unscheduledPods) {
							try {
								...
								// First-Fit: Select first node with sufficient resources
								NodeSelection selection = selectNodeFirstFit(nodes, pod);
								...
								SchedulingMetrics metrics = new SchedulingMetrics(pod.getMetadata().getName(),
										totalTime, selectionTime, bindingTime, selection.nodeName,
	...
	/**
	 * First-Fit selection: Return FIRST node with sufficient resources
	 */
	private static NodeSelection selectNodeFirstFit(List<V1Node> nodes, V1Pod pod) throws ApiException {
		...
		// Iterate through nodes - return FIRST that fits
		for (V1Node node : nodes) {
			try {
				...
				// First-Fit: Return FIRST node that fits
				if (availableCpu >= requiredCpu && availableMemory >= requiredMemory) {
					...
					logger.info(String.format("Selected node %s (First-Fit) - CPU util: %.2f%%, Mem util: %.2f%%",
							node.getMetadata().getName(), cpuUtilAfter * 100, memUtilAfter * 100));

					return new NodeSelection(node.getMetadata().getName(), cpuUtilAfter, memUtilAfter);
				}
	...
	private static void storeSchedulingMetrics(V1Pod pod, SchedulingMetrics metrics) {
		try {
			V1Patch patch = new V1Patch(String.format("{\"metadata\":{\"annotations\":{"
					+ "\"scheduler.algorithm\":\"First-Fit\"," + "\"scheduler.node\":\"%s\","
					+ "\"scheduler.metrics.totalTimeMs\":\"%d\"," + "\"scheduler.metrics.selectionTimeMs\":\"%d\","
					+ "\"scheduler.metrics.bindingTimeMs\":\"%d\"," + "\"scheduler.metrics.cpuUtilization\":\"%.4f\","
					+ "\"scheduler.metrics.memoryUtilization\":\"%.4f\"" + "}}}", metrics.selectedNode,
					metrics.totalTime.toMillis(), metrics.selectionTime.toMillis(), metrics.bindingTime.toMillis(),
					metrics.nodeUtilizationCpu, metrics.nodeUtilizationMemory));
	...
	private static List<V1Pod> getUnscheduledPods(CoreV1Api api) throws ApiException {
		...
		return podList.getItems().stream()
				.filter(pod -> pod.getSpec().getNodeName() == null && pod.getSpec().getSchedulerName() != null
						&& pod.getSpec().getSchedulerName().equals("first-fit-scheduler"))
		...
		try {
			...
			CoreV1Event event = new CoreV1Event()
					.metadata(new V1ObjectMeta()
							.name("first-fit-scheduled-" + pod.getMetadata().getName() + "-"
									+ UUID.randomUUID().toString().substring(0, 8))
							.namespace(pod.getMetadata().getNamespace()))
					.type("Normal").reason("Scheduled").message("Pod scheduled by First-Fit scheduler")
					...
					.source(new V1EventSource().component("first-fit-scheduler"));
